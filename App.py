"""
ChatSolveAI — Streamlit frontend (v2.1) — Chat UI only.
"""
...
def _session_id_from_url():
    ...
    return sid if sid and len(sid) <= 128 else None
...
def _adopt_url_session():
    ...
    if not url_id or url_id == st.session_state.get("session_id"):
        return
...
def _init_state():
    url_id = _session_id_from_url()
    defaults = {
        "session_id": url_id or str(uuid.uuid4()), "conv_id": str(uuid.uuid4())[:8],
        "messages": [], "last_sources": [], "last_meta": {},
        "pending_query": None, "pending_append_user": True, "history_loaded_for": None,
    }
...
def call_chat(query):
    for attempt in range(2):
        try:
            ...
            st.error(f"Network error: {e}"); return None
    return None
...
def call_chat_stream(query, output_box=None):
    ...
    try:
        with requests.post(f"{API_URL}/chat/stream",
            json={"session_id":st.session_state.session_id,"query":query},
            headers=_api_headers(), timeout=90, stream=True) as r:
            ...
            for line in r.iter_lines(decode_unicode=True):
                ...
                if "token" in ev:
                    ...
                    final = ev
    except requests.RequestException as e: err = str(e)
    if final:
...
def call_history(sid):
    try:
        r = requests.get(f"{API_URL}/history/{sid}", timeout=15)
        if r.status_code == 404: return []
        if r.ok: return r.json().get("messages",[])
    except: pass
    return None

def call_suggest(answer):
    try:
        r = requests.post(f"{API_URL}/suggest", json={"last_answer":answer,"n":3},
...
def call_feedback(q,a,rating):
    try:
        r = requests.post(f"{API_URL}/feedback",
            json={"session_id":st.session_state.session_id,"query":q,"answer":a,"rating":rating},
            headers=_api_headers(), timeout=10)
        return r.ok
    except: return False

# ═════════════════════════════════════════════════════════════════════
# Callbacks & helpers (exact same as before, only chat-related)
# ═════════════════════════════════════════════════════════════════════
def _queue_query(q):
...
def render_meta(meta):
    ...
    st.markdown(f'<div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap;">{pills}</div>',
                unsafe_allow_html=True)
    st.markdown(f'<div class="meter-wrap"><div class="meter-fill" style="width:{conf*100:.0f}%"></div></div>',
                unsafe_allow_html=True)

def _similarity_from_l2(l2):
    return max(0, min(1, 1 - l2**2/2))
    ...
    with st.expander(f"📚 Sources ({len(sources)})", expanded=False):
        for i, src in enumerate(sources[:4]):
            ...
            if src.get("score") is not None:
                bits.append(f'similarity: {_similarity_from_l2(float(src["score"])):.2f}')
            ...
            st.markdown(f'<div class="src-card {cls}"><strong>{rank}</strong> — {src["content"][:220]}'
                        f'{"…" if len(src["content"])>220 else ""}<div class="src-meta">{line}</div></div>',
                        unsafe_allow_html=True)

def build_transcript_md():
    lines = [f"# ChatSolveAI Conversation\n_Session `{st.session_state.session_id}` · "
    ...
    for m in st.session_state.messages:
        ...
        if m["role"]=="assistant" and m.get("meta"):
            meta = m["meta"]
            lines.append(f"> intent: {meta.get('intent','general')} · "
...
def submit_query(query, append_user=True):
    ...
    if not result:
        if append_user and st.session_state.messages and \
...
def _perform_full_reset():
    ...
    for k in list(st.session_state.keys()):
        if isinstance(k,str) and k.startswith(("fb_","up_","down_","fu_","chip_","followup_")):
...
if st.session_state.pending_query:
    if healthy:
        ...
        append = bool(st.session_state.get("pending_append_user", True))
        st.session_state.pending_query = None
        st.session_state.pending_append_user = True
...
if not st.session_state.messages:
    ...
    for i, (emoji, q) in enumerate(EXAMPLE_QUESTIONS):
        with cols[i % 2]:
            ...
            st.button(f"{emoji}   {q}", key=f"chip_{conv}_{i}",
                    ...
                    if st.session_state.get(fb) is None:
                        ...
                        if rating=="down":
                            if st.button("Regenerate", key=f"regen_{conv_id}_{idx}"):
                                ...
                                del st.session_state[fb]; st.rerun()
    # follow-ups
    with st.container():
        if msgs[-1]["role"]=="assistant":
            fu = msgs[-1].get("followups") or []
            if fu:
                st.markdown('<div style="margin-top:14px;font-weight:600;color:#c9b8ff;">💡 Suggested follow‑ups</div>',
                            unsafe_allow_html=True)
